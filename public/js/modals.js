...
function openModal(i) {
  ...
  // Graphic design → go to full gallery page
  if (p.tab === 'graphic') {
    openGalleryPage(i);
    return;
  }

  // Video editing → go to full video page
  if (p.tab === 'video') {
    openVideoPage(i);
    return;
  }

  // Web and UI/UX → use the modal
  ...
  // Live link — only for web projects
  ...
  if (p.tab === 'web' && linkEl) {
    ...
    linkEl.innerHTML = ''; // hide for uiux
  }
...
// ── Opens the graphic design full gallery page
function openGalleryPage(i) {
  const p = PROJECTS[i];
  document.getElementById('graphicTitle').textContent = p.title;
  document.getElementById('graphicDesc').textContent = p.desc;
  document.getElementById('graphicTags').innerHTML = p.tags.map(t => `<span class="tag">${t}</span>`).join('');

  const grid = document.getElementById('graphicGallery');
  grid.innerHTML = p.images.map(src => `
    <div class="gallery-page-item">
      <img src="${src}" alt="${p.title}"
        onerror="this.parentElement.innerHTML='<div class=gallery-page-placeholder>✦</div>'">
    </div>
  `).join('');
...
// ── Opens the video editing full page
function openVideoPage(i) {
  const p = PROJECTS[i];
  document.getElementById('videoTitle').textContent = p.title;
  document.getElementById('videoDesc').textContent = p.desc;
  document.getElementById('videoTags').innerHTML = p.tags.map(t => `<span class="tag">${t}</span>`).join('');

  const grid = document.getElementById('videoGallery');
  grid.innerHTML = p.videoUrl ?
    `<div class="gallery-page-video-wrap">
        <iframe src="${p.videoUrl}" frameborder="0" allowfullscreen
          allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture">
        </iframe>
      </div>` :
    `<div class="gallery-page-placeholder" style="height:300px">
        🎬 Video coming soon
      </div>`;
